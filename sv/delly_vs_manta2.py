...
Modified from version on on March 10, 2018 to output file as vcf file
"""
...
def vcf2SVPosition(vcf_file):
    ...
    for variant in cyvcf2.VCF(vcf_file):
        ...
        bp_dict[variant_type].append((bp1, bp2, variant))
...
def compare_breakpoints(vcf1, vcf2, distance_threshold, circosDataFile, filteredOutput, sampleName, filteredVCF):
    vcf1_positions = vcf2SVPosition(vcf1)
    vcf2_positions = vcf2SVPosition(vcf2)
    
    os.system('bcftools view -h ' + vcf1 + ' > ' + filteredVCF)

    with open(filteredVCF , 'a') as h:
        with open(filteredOutput, 'w') as g:
                ...
                for svtype in ['BND', 'INV', 'DUP', 'DEL', 'INS']:
                    ...
                    for vcf1_sv in vcf1_positions[svtype]:
                        ...
                        if status == True:
                            ...
                            g.write(f'{vcf1_sv[0]}\t{vcf1_sv[1]}\t{svtype}\t{sampleName}\n')
                            h.write( str(variant_vcf1))

#    with open(filteredVCF + '.tmp', 'r') as h:
#        with open(filteredVCF, 'w') as k:
#            for line in h:
#                if line != '\n':
#                    k.write(line)
...
def main():
    ...
    filteredOutput = os.path.join(outputDIR, sampleName + '.filtered_sv.tsv')
    filteredVCF = os.path.join(outputDIR, sampleName + '.intersectFilter.vcf')
    compare_breakpoints(delly, manta, 1000, circosDataFile, filteredOutput, sampleName, filteredVCF)
    print(f'Output Circos Data file is written: {circosDataFile}')
    print(f'Output Filtered Variant TSV file is written: {filteredOutput}')
    return 0
